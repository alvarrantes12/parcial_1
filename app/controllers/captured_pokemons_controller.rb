class CapturedPokemonsController < ApplicationController
  ...
  def create
    @captured_pokemon = CapturedPokemon.new(captured_pokemon_params)
      ...
      if @captured_pokemon.save
        redirect_to captured_pokemon_url(@captured_pokemon), notice: t('activerecord.modules.states.created')
      else
  ...
  def update
      if @captured_pokemon.update(captured_pokemon_params)
        redirect_to captured_pokemon_url(@captured_pokemon), notice: t('activerecord.modules.states.updated') 
      else
  ...
  def destroy
    ...
    redirect_to captured_pokemons_url, notice: t('activerecord.modules.states.deleted') 
  end
  ...
  private
    ...
    def set_pokemon_trainers
      @pokemon_trainers = PokemonTrainer.all.map { |pokemon_trainer| ["#{pokemon_trainer.first_name}", pokemon_trainer.id] }
    end
