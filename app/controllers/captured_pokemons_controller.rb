class CapturedPokemonsController < ApplicationController
  ...
  def create
    @captured_pokemon = CapturedPokemon.new(captured_pokemon_params)
      ...
      if @captured_pokemon.save
        redirect_to captured_pokemon_url(@captured_pokemon), notice: t('activerecord.created') 
      else
  ...
  def update
      ...
      if @captured_pokemon.update(captured_pokemon_params)
        redirect_to captured_pokemon_url(@captured_pokemon), notice: t('activerecord.update') 
      else
  ...
  def destroy
    @captured_pokemon.destroy
      ...
      redirect_to captured_pokemons_url, notice: t('activerecord.delete') 
  end
