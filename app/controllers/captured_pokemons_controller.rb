class CapturedPokemonsController < ApplicationController
  ...
  def create
    ...
    if @captured_pokemon.save
      redirect_to captured_pokemon_url(@captured_pokemon), notice: t("faq.created")
    else
  ...
  def update
    if @captured_pokemon.update(captured_pokemon_params)
      redirect_to captured_pokemon_url(@captured_pokemon), notice: t("faq.updated")
    else
  ...
  def destroy
    ...
    redirect_to captured_pokemons_url, notice: t("faq.destroyed")
  end
